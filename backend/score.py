...
from src.gemini_key_manager import get_gemini_key_manager, initialize_gemini_keys
...
# Disable Vertex AI for local development - chỉ sử dụng Gemini API key
...
async def extract_knowledge_graph_from_file(
    ...
    try:
        ...
        if source_type == 'local file':
            ...
            uri_latency, result = await extract_graph_from_file_local_file(uri, userName, password, database, model, merged_file_path, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)

        elif source_type == 's3 bucket' and source_url:
            uri_latency, result = await extract_graph_from_file_s3(uri, userName, password, database, model, source_url, aws_access_key_id, aws_secret_access_key, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)
        
        elif source_type == 'web-url':
            uri_latency, result = await extract_graph_from_web_page(uri, userName, password, database, model, source_url, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)

        elif source_type == 'youtube' and source_url:
            uri_latency, result = await extract_graph_from_file_youtube(uri, userName, password, database, model, source_url, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)

        elif source_type == 'Wikipedia' and wiki_query:
            uri_latency, result = await extract_graph_from_file_Wikipedia(uri, userName, password, database, model, wiki_query, language, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)

        elif source_type == 'gcs bucket' and gcs_bucket_name:
            uri_latency, result = await extract_graph_from_file_gcs(uri, userName, password, database, model, gcs_project_id, gcs_bucket_name, gcs_bucket_folder, gcs_blob_filename, access_token, file_name, allowedNodes, allowedRelationship, token_chunk_size, chunk_overlap, chunks_to_combine, retry_condition, additional_instructions, schema)
        else:
            return create_api_response('Failed',message='source_type is other than accepted source')
        ...
        # Handle case where node_detail is None or empty
        created_time = None
        if node_detail and len(node_detail) > 0 and 'created_time' in node_detail[0]:
            created_time = node_detail[0]['created_time']
        ...
        json_obj = {'api_name':'extract','message':error_message,'file_created_at':formatted_time(created_time),'error_message':error_message, 'file_name': file_name,'status':'Completed',
        ...
        json_obj = {'api_name':'extract','message':message,'file_created_at':formatted_time(created_time),'error_message':error_message, 'file_name': file_name,'status':'Failed',
...
# Auto-connect to Neo4j on startup
async def startup_event():
    """Startup event to auto-connect to Neo4j and initialize Gemini keys"""
    print("=" * 60)
    print("🚀 LLM GRAPH BUILDER BACKEND STARTING...")
    print("=" * 60)
    
    # Initialize Gemini API keys
    try:
        print("🔑 Initializing Gemini API key rotation...")
        key_manager = initialize_gemini_keys()
        print(f"✅ Initialized {len(key_manager.api_keys)} Gemini API keys")
        print(f"🔄 Current key: {key_manager.get_current_key()[:10]}...")
        print(f"✅ {len(key_manager.api_keys)} Gemini API keys ready for use")
            
    except Exception as e:
        print(f"⚠️  Gemini API key initialization failed: {e}")
        print("💡 Backend will still start but Gemini models may not work")
    ...
    try:
        ...
        if all([neo4j_uri, neo4j_username, neo4j_password]):
            ...
            # Test query
            result = graph.query("RETURN 'Backend connected to Neo4j!' as message")
            print(f"✅ Neo4j: {result[0]['message']}")
            
            # Get database info
            node_count = graph.query("MATCH (n) RETURN count(n) as count")[0]['count']
            print(f"📊 Current nodes: {node_count:,}")
            
            print("✅ Backend is ready to process documents!")
            
        else:
            print("⚠️  Neo4j credentials not found - will connect on first request")
            
    except Exception as e:
        print(f"⚠️  Neo4j connection test failed: {e}")
        print("💡 Backend will still start - connection will be attempted on first request")
...
@app.get("/gemini_keys/status")
async def gemini_keys_status():
    """Get status of Gemini API keys"""
    try:
        key_manager = get_gemini_key_manager()
        
        status = {
            "total_keys": len(key_manager.api_keys),
            "available_keys": key_manager.get_available_key_count(),
            "failed_keys": len(key_manager.failed_keys),
            "current_key_prefix": key_manager.get_current_key()[:10] + "...",
            "rotation_interval": key_manager.rotation_interval,
            "last_rotation": key_manager.last_rotation_time,
            "should_rotate": key_manager.should_rotate()
        }
        
        return create_api_response('Success', data=status, message="Gemini API keys status retrieved")
        
    except Exception as e:
        error_message = str(e)
        logging.exception(f'Exception in gemini_keys_status: {error_message}')
        return create_api_response('Failed', message="Unable to get Gemini keys status", error=error_message)

@app.post("/gemini_keys/rotate")
async def rotate_gemini_key():
    """Manually rotate to next Gemini API key"""
    try:
        key_manager = get_gemini_key_manager()
        ...
        return create_api_response('Success', 
                                 message=f"Rotated from {old_key}... to {new_key}...",
                                 data={"old_key_prefix": old_key + "...", "new_key_prefix": new_key + "..."})
        
    except Exception as e:
        error_message = str(e)
        logging.exception(f'Exception in rotate_gemini_key: {error_message}')
        return create_api_response('Failed', message="Unable to rotate Gemini key", error=error_message)

@app.post("/gemini_keys/reset_failed")
async def reset_failed_gemini_keys():
    """Reset all failed Gemini API keys (give them another chance)"""
    try:
        key_manager = get_gemini_key_manager()
        failed_count = len(key_manager.failed_keys)
        key_manager.reset_failed_keys()
        
        return create_api_response('Success', 
                                 message=f"Reset {failed_count} failed keys",
                                 data={"reset_count": failed_count})
        
    except Exception as e:
        error_message = str(e)
        logging.exception(f'Exception in reset_failed_gemini_keys: {error_message}')
        return create_api_response('Failed', message="Unable to reset failed Gemini keys", error=error_message)