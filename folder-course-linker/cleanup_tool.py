"""
Cleanup Tool for Folder Course Linker
Xóa tất cả folder nodes và relationships được tạo bởi tool
"""
import logging
...
def main():
    """Main function - Tự động cleanup tất cả"""
    logging.basicConfig(level=logging.INFO)
    cleanup = CleanupTool()
    
    try:
        print("=" * 60)
        print("🧹 FOLDER COURSE LINKER - AUTO CLEANUP TOOL")
        print("=" * 60)
        ...
        print(f"\n🗑️ AUTO CLEANUP: Deleting ALL data...")
        print(f"   - Will delete {stats['relationships_count']} relationships")
        print(f"   - Will delete {stats['folder_nodes_count']} folder nodes")
        print(f"   - Folder types: {stats['folder_types']}")
        
        # Tự động cleanup tất cả
